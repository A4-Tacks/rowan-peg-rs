...
::peg::parser!(pub grammar parser<'b>(state: &'b ::rowan_peg_utils::ParseState<'input>) for str {
    ...
    rule int() = ()(()(g:({state.guard_none()})((g:({state.quiet().guard_token(INT)}) s:$(((()(g:({state.guard_none()})((g:({state.guard_none()})(#{classes!("0-9")}){g.accept_none()})+){g.accept_none()})))) {g.accept_token(s)})){g.accept_none()}) / expected!("int"))
    rule text() = ()(()(g:({state.guard_none()})((g:({state.quiet().guard_token(TEXT)}) s:$(((()(g:({state.guard_none()})((g:({state.guard_none()})(#{classes!("a-zA-Z_-")}){g.accept_none()})+){g.accept_none()})))) {g.accept_token(s)})){g.accept_none()}) / expected!("text"))
